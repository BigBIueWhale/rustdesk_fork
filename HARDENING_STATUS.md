...
| Verification/build authority | Focused source/model gates remain supplementary. R-S11dh's authenticated ordinary-user, zero-NIC QEMU smoke proves direct boot, guest-only Docker, private Unix control, selected root/foreign refusal, bounded containers, listener invariance, finality, and joined cleanup; its script entries have no host-Docker fallback. Those authority probes do not execute most real Dart/FRB, advisory, Android, Apple, full-peer, certified Debian/Windows, or RustDesk product workloads. R-S11cj's distinct outbound-only VM has real pinned-HTTPS/cache-transport authority evidence and, at `cd3891fae9c4b9f98b14aabc246d18f35af2d74b`, successful and expected-failure runs proving the acquisition principal inherits the exact descriptor budget needed for private-workspace finality. Same-generation bootstrap candidate capture was added at `4ea5ecbf2f632b617df901b2767fb584ddb2abd5`; successive real runs corrected final-pin use during discovery, embedded provenance modes, and classic-builder `Config.Image` bookkeeping while retaining exact runtime/rootfs equality. At clean pushed commit `53b16fe87de926a4281ccfd1c4e5449f95519798`, the authority smoke exercised the exact 16 MiB-plus-one-byte result fixture and completed with listeners unchanged and cleanup joined. The subsequent real Android candidate passed discovery, non-root inspection, metadata seal, seal verification, and bounded archive capture, then correctly rejected the tag-created legacy `repositories` metadata; no candidate was published. Current source deletes the private-candidate tag path, saves by immutable image ID, and requires an untagged content-addressed archive. Focused checks are green; the real candidate rerun remains pending. Complete cache/producers, exact full-peer source/input/output transport, prepared toolchain/image reproduction, fresh independent verifier-image rebuild, native targets, and cold release remain open. |
...
- **R-S11cj/R-S11e-102 — isolated online-acquisition VM and non-root producer authority — SOURCE,
  ...
  `repositories` file, and one exact config/manifest/image identity. The behavioral provenance fixture rejects the
  formerly accepted tagged candidate and accepts the same untagged bytes both during discovery and after those
  extracted identities become promotion pins. Focused gates are green, but the real Android candidate rerun remains
  required before the clean-cache prerequisite is operational. The complete cache, real producer workloads and
