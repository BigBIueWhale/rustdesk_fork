...
| R-P2/R-P10 two-key `secretbox` rewrite | **PARTIAL — wire-VERIFIED core** | `5ec40de`; `hbb_common::cpace::DirectionalCipher` (two per-direction keys + per-direction counters) is the R-P10 fix, **tested over a loopback socket** (round-trips both directions; single-key collapse rejected). REMAINING: flip `FramedStream`/`tcp::Encrypt` from the single-key tuple to `DirectionalCipher`, update `Stream`/`WsFramedStream` set_key, remove the single-key `set_key` (R-A6) — the choke-point cutover below |
