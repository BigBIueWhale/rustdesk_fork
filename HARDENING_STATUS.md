...
verified, but the viewer, the GUI, the lockdown-feature retirement, and the external audit remain.
...
| R-R2b one binary, both roles — harden unconditionally, no build-profile split | **PARTIAL — re-based** | Keeps RustDesk's single binary (viewer + `--server` by argv); surface reduced by deletion, policy unconditional. Scripts now build one `.deb`/`.apk`/`.exe` with hwcodec/vram dropped (software codec). REMAINING: retire the `lockdown` feature so PINNED_SETTINGS / R-A1 / R-A3 / R-A4 are unconditional — see the `TODO(one-binary)` markers |
...
5. R-R2b lockdown-feature retirement, R-R3 audit, §19 GUI sweep, R-R2 CI prune, smoke-tests (R-B4).
