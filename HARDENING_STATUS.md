...
- **T2 — Settings unlock/password (cavity 2):** drop the dead elevation `_lock` gate → Set-Password settable on Win/Linux; free the 5 trapped non-pinned prefs; R-G1-render the 15 pinned toggles read-only "set by policy"; delete `hide_cm()`; fix the home pencil.
- **T4 — Android (cavity 4 + N1/F1/F5):** honor `EXT_INIT_FROM_BOOT` (decouple boot from capture — the BR-17 core fix); tap-to-consent-on-connect (hold session + retry `startCapture` on grant); close the N1/F1 orphaned-listener race; F5 projection release on `onDestroy`; excise/pin the scam-dialog; gate the ServerInfo "Reachable" fact; relabel "Start service"→"Start screen sharing" (BR-15).
- **T1 — Windows resilience + honest status (cavity 1 + N3):** `SetServiceFailureActions` crash-restart parity (systemd/launchd parity; respects R-X9/R-X10) + the Linux N3 start-limit; remove the tray `sc delete` "Stop service" + add a non-destructive quit; honest desktop "Listening" via `permanent-password-set`+real-bind (BR-4); empty-PRS fail-loud/repair.
- **T3 — UI coherence remainder (cavity 3):** the directly-visible Linux "Allow linux headless" R-G1 toggle; R-G8 branding residuals (minor). (Dead WaylandCard + ~1,200-line account/AB compile-out = DEFERRED hygiene.)
...
```text
1c3203904909cbc51439fe09d8a66247738bc26a926bbe1e9cd34627e1d964a4  requirements.html
```
