...
| §18 R-SV3/SV4 sovereignty (kill version-check egress + `test_nat_type` probe) | **PARTIAL — egress excised (VERIFIED)** | `fa56d66`; the two phone-home workers DELETED: `do_check_software_update` (the `api.rustdesk.com/version` POST, R-SV3) and `test_nat_type_` (the rendezvous NAT probe + its `cu` re-home, R-SV4). `check_software_update`/`test_nat_type` are now no-ops (egress gone, not policy-gated). Compile-verified. The auto-updater was R-X1. REMAINING: token-absent removal of the residual neutralized callers (main.rs/cli.rs/flutter_ffi/mediator) comes with R-D4 |
