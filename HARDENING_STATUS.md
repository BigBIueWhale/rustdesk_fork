...
| R-S1/R-P4/R-P14 choke-point integration in `create_tcp_connection` | **PARTIAL — driver + both foundations VERIFIED** | `5ec40de` drivers (`run_responder`/`run_initiator`, full R-P14a/b/c, cpace_it-verified) + the two foundations the choke point needs are now in & tested: `0108349` `FramedStream::set_session_keys` (the keying call) and `ef8fa72` `Config::get_permanent_password_prs` (the PRS source). REMAINING (the actual wiring, main-crate compile-verifiable now): in `create_tcp_connection`/`client.rs` run the driver over the stream + `set_session_keys`, drop the `if secure` SignedId↔PublicKey bootstrap + `box_`/`sign` (R-P5/§8), delete `validate_password` + set `authorized` only on the KEYED edge (R-S2/R-A1/A2), adapt `common.rs` secure_tcp. (Driver takes `&mut FramedStream`; the choke point's `Stream` is `Tcp` on the direct path — minor plumbing or a `&mut Stream` signature.) |
