...
| R-B9/B10/B11 build harness — manifest, helpers, provision/cleanup, online-fetch | **PARTIAL** | `pins.env` + `lib.sh` + `host-provision.sh`/`cleanup.sh` (R-B11) + `online-fetch.sh` (R-B10, SHA-verified fetch incl. `cargo vendor --locked`). The build-script *bodies* (`build-debian/android.sh`, `build-windows.ps1`, `provision-windows-vm.sh` — upstream's exact 1.4.7 commands) = TODO/`DEFER-BUILD`. SHA-256 digests stay the fail-closed R-B12 sentinel (audited bootstrap is online) |
