...
The server runtime-smoke entry now uses that same sole VM authority. Before product input or scratch access it
refuses root, authenticates the fixed guest entry, binds the repository Docker-version pin to the VM marker, and
routes every Docker operation through the guest-private socket and root-owned read-only configuration with pre/post
daemon-generation replay. The KVM smoke behaviorally proved root and foreign-principal refusal plus the authorized
UID/GID-4000 Docker client/server operation, while the outer harness again proved zero VM NICs, unchanged host
listeners, and joined residue-free cleanup in 16–17 seconds. The Xvfb preparation path also no longer creates a
bridge or downloads packages: it is network-none and accepts only a separately authenticated read-only offline
package closure. No development image, source/vendor closure, Xvfb package closure, RustDesk binary, or product
stage was admitted to this VM run, so the actual server build, protocol/video stages, root fixtures, and full
...
Unix serial/QMP channels, bounded complete serial capture, and before/during/after host-listener comparison. The
authenticated image's exact root partition/filesystem and kernel/initramfs bytes are pinned; a nonroot, offline,
no-mount derivation uses fixed `qemu-img`/`sgdisk`/`debugfs` tools and descriptor-safe scratch cleanup to publish
one exact private cache. QEMU direct-boots retained descriptors for those bytes and supplies runtime-only systemd
masks for the absent-network wait and SSH units; the guest proves its exact kernel release, complete command line,
and generator-created masks. Repeated complete passes finished within 12.3 seconds wall time under a 90-second bound,
two independent cold derivations produced the same pinned bytes in under four seconds, and a controlled interruption
joined the exact QEMU/capture processes without a listener delta before its exact retained evidence was reconciled.
In the disposable guest, the rootful daemon has no bridge, IP forwarding, iptables/ip6tables mutation, masquerade,
...
Every image inspection or container launch expressed directly in the main verifier, Dart verifier, or FRB generator now revalidates
that record before and after one empty-environment call to the fixed guest client/socket/configuration. The previous
`initialize_local_docker_authority` and `/var/run/docker.sock` route is absent from all three entries. Real KVM execution
...
The expanded KVM smoke runs both actual child entries as UID/GID 4000. `frb-codegen.sh
--self-test-vm-authority` and `dart-verify.sh --self-test-vm-authority` each perform a preflight-wrapped fixed-client
Docker request with client/daemon generation replay before and after; the Dart entry also invokes and validates the
FRB entry as its real nested child. VM root is refused by the Dart entry before repository helpers are sourced, and
UID/GID 4001 is refused at the guest-socket boundary for both. After the Dart
migration, two consecutive complete passes finished in 15–16 seconds with unchanged host listeners and joined
residue-free cleanup; the guest-side focused gate rejected all 128 deliberate mutations after obsolete host-Docker assertions
and mutations were deleted. This is executable entry, launcher-chain, and supplementary source-gate evidence, not
a full Dart transaction, FRB generation, or full-verifier result. The pinned devcheck archive has no published
release/package source; the certified deb-builder archive, local Cargo vendor/config, complete `online/` source
input, and verifier image are absent from this smoke, so image-provenance inspection, the real code-generation and
Dart containers, the exact source/offline-input/output transaction, and the full verifier workload have not run.
Other shared build, scanner, provenance, release-orchestration, and smoke launchers still address root-equivalent
host Docker and remain forbidden here. The complete single-topology launcher migration, reproducible prepared
verifier toolchain/image, fresh independent image rebuild, bounded read-only inputs, validated outputs, and a
current full gate remain STOP-SHIP.
...
R-S11cq and Appendix C #244 now state only the timeless Android NDK extraction/publication contract
and risk disposition. The historical failure narrative, `FIX` badge, implementation diary, mutation
count, and one-off archive receipt were removed, reducing `requirements.html` from 2,267,202 to
2,264,831 bytes. The R-S11cq ledger was consolidated to current behavior, bounded historical evidence,
and explicit current gaps. The focused checker lost its mutation catalog and all requirements/ledger/
workspace prose coupling; the workspace verifier's duplicate validator, dispatch, mutations, and two
loaders were deleted. Review also found that a verified tree interrupted after its final rename but
before its root-mode seal could not recover, and that publication changed the root mode by pathname.
The corrected transaction uses the journaled inode and archive-derived tree digest for a descriptor-
bound root transition, exact rollback, and post-rename/pre-seal recovery, with real filesystem crash
and changed-tree regressions. Including this architecture record, the ledger moved from 1,056,968 to
1,051,111 bytes, or 350,371 conservative tokens by the documented method. No exact-current full pinned-
archive, package, or Android-device result is claimed.

R-S11cn/R-S11fy and Appendix C #233/#333 now state the timeless Pub-cache acquisition,
publication, recovery, and stale-replacement contracts instead of implementation history, `FIX`
status, verifier receipts, mutation counts, and repeated unrelated release disclaimers. This reduced
`requirements.html` by 4,647 bytes. The two Pub ledger entries were compressed to current source
behavior, retained evidence, and explicit open native/release gaps; including this architecture record,
the full ledger fell by 9,598 bytes to 1,066,574 bytes, or 355,525 conservative tokens by the documented
method. The workspace verifier's
duplicate Pub validator, dispatch, mutation block, and two dedicated loaders were deleted rather
than updated, removing 530 lines and 24,762 bytes. The surviving focused verifier no longer reads
requirements, ledger, or workspace prose and retains the product/source boundary checks. Review also
found and corrected three real transaction defects: the durable selection preceded candidate
synchronization, nonreplacement recovery could classify a moved unselected inode as published and
did not finish a selected post-rename root seal, and the displaced-tree record omitted exact UID/GID/
mode commitment. State v3, exact legacy-v2 policy, and the current confined filesystem crash tests
cover those corrections at the layers described in R-S11cn/R-S11fy. This is one coherent cleanup and
product-correction slice, not completion of the documentation work item or release evidence.
...
R-S11ds through R-S11eb and Appendix C #272–#281 now state only the durable Windows domain/run-state, cross-platform
publication, privileged service protocol, PulseAudio, whiteboard, desktop-URL, and mobile-session contracts; their
defect diaries, source-review claims, verifier inventories, `FIX` badges, ledger bookkeeping, and current evidence
disclaimers are removed. The Windows focused and independent validators retain their cleanup/publication code and
filesystem-fixture checks but no longer parse R-S11ds/R-S11dt/R-S11du, Appendix #272–#274, or their status wording,
or mutate those documents: 354 lines and 15 non-product mutations are deleted. The stale 111-line R-S11ds and
100-line R-S11dt ledger diaries are replaced by concise current dispositions; R-S11dt is aligned with R-S11gl's
all-conclusive-outcome bulk cleanup. The focused Windows verifier is now 198,016 bytes/4,554 lines, the workspace
verifier is 1,893,553 bytes/45,349 lines, and `requirements.html` is 2,184,801 bytes. The normal macOS credential
verifier also no longer loads or mutates requirements, this ledger, shared/Apple gate wording, or the independent
...
Appendix C #292 and the R-S11e-171 ledger entry are deleted outright. They recorded only a historical false failure
caused by a source verifier selecting the first generic loop in the Windows provisioner; they described no product
vulnerability, runtime authority, current residual, or requirement not already owned by R-S11ch and the current
R-S11e-100 disposition. The normative document falls by 1,495 bytes and the obsolete 59-line ledger diary is gone.
The focused Windows-helper verifier loses its Appendix/status wording checks and mutations, reducing it by 695
bytes/14 lines, but retains the exact completion-phase extraction, marker-before-final-hash-before-acceptance check,
and its product-source negative mutation. The workspace verifier loses 1,219 bytes/33 lines of documentation,
focused-verifier-internal, and duplicate completion-phase checks; it still binds the dedicated focused gate's source
ownership and shared-gate wiring, while the separate golden-domain validator retains marker, final hash, exact-domain
retirement, and success ordering. No product source or native target changed or ran. Including this architecture
...
| Verification/build authority | Focused source/model gates remain supplementary. R-S11dh's authenticated ordinary-user, zero-NIC QEMU smoke proves direct boot, guest-only Docker, private Unix control, selected root/foreign refusal, bounded containers, listener invariance, finality, and joined cleanup; its script entries have no host-Docker fallback. Those authority probes do not execute most real Dart/FRB, advisory, Android, Apple, full-peer, certified Debian/Windows, or RustDesk product workloads. R-S11cj's distinct outbound-only VM has real pinned-HTTPS/cache-transport authority evidence and, at `cd3891fae9c4b9f98b14aabc246d18f35af2d74b`, successful and expected-failure runs proving the acquisition principal inherits the exact descriptor budget needed for private-workspace finality. Same-generation bootstrap candidate capture was added at `4ea5ecbf2f632b617df901b2767fb584ddb2abd5`, but its first real Android run failed because networked refresh still required the old final package-manifest pin. Commit `84d79bb61c93c13b61d27a5919b09724cd899b1f` separated discovery from final pins; its first VM run completed the real package install and then correctly failed nonroot inspection because the embedded recipe inherited mode 0600 from the private source checkout. Commit `21db3821d3fb9802e8bddc5278c8b0ba2813aa20` corrected those modes; the next VM run passed discovery inspection and built the seal, but the exact configuration-equality check rejected an additional legacy-builder transformation. Diagnostic commit `ee826671edb7dd2024ff36e4c250244764eeb70a` proved the sole difference was Moby classic-builder `Config.Image` source-image bookkeeping, which is absent from OCI execution parameters. Current source excludes only that key while requiring every runtime configuration field, rootfs layer identity, and label invariant exactly; another clean-commit run is pending. Complete cache/producers, exact full-peer source/input/output transport, prepared toolchain/image reproduction, fresh independent verifier-image rebuild, native targets, and cold release remain open. |
...
| Build/test execution authority (R-S11dh) | **STOP-SHIP; FAST NO-NIC AUTHORITY SMOKES AND THE DISTINCT ONLINE-ACQUISITION AUTHORITY SMOKE ARE GREEN, BUT PRODUCT AND RELEASE WORKLOADS REMAIN OPEN.** The R-S11dh entry paths admit only the authenticated ordinary-user QEMU/direct-boot/guest-Docker authority and have no host-Docker fallback; the focused entries behaviorally refuse applicable root/foreign callers and exercise real bounded guest containers. R-S11cj separately executes networked acquisition inside ordinary-user QEMU with three exact rootless virtiofs exports, TCP-only guest/container egress, no host forwarding, and unchanged host listeners. Its pinned-HTTPS authority run remains green; at `cd3891fae9c4b9f98b14aabc246d18f35af2d74b`, another successful 16-second run validated the exact 524,544-descriptor acquisition budget, while a 27-second expected missing-image transaction reached clean private-workspace finality and joined the VM processes with listeners unchanged. This proves execution-authority and cleanup mechanics only. Commit `4ea5ecbf2f632b617df901b2767fb584ddb2abd5` removed the cross-VM Docker-store dependency, but the first real Android candidate run then failed closed on an obsolete final package-manifest pin before capture. Commit `84d79bb61c93c13b61d27a5919b09724cd899b1f` made final recipe/package pins review outputs and added the validated metadata-only seal; its first exact VM run exposed unreadable mode-0600 provenance copied from the private checkout. Commit `21db3821d3fb9802e8bddc5278c8b0ba2813aa20` corrected those modes and reached the metadata seal. Diagnostic commit `ee826671edb7dd2024ff36e4c250244764eeb70a` proved its sole additional field was classic-builder `Config.Image`, which records build source rather than OCI runtime behavior. Current source excludes exactly that bookkeeping key and retains exact equality for every runtime field, rootfs layer, and label; another clean-commit run is required before the clean-cache prerequisite can be called operational. Real Dart/FRB and advisory scans, normal Android signing/build/Gradle/Rust workloads, Apple source work, full-peer presentation, certified Debian/Windows workloads, cold artifacts, prepared verifier inputs, fresh independent image reproduction, and external review remain open. Entry gates are not product, artifact, scanner, native-platform, or full-release evidence. |
...
- **R-S11cj/R-S11e-102 — isolated online-acquisition VM and non-root producer authority — SOURCE,
  ...
  systemd-image cache, or the 16 MiB-bounded result root. Each launcher requires Landlock ABI 8, permits only
...
- **R-S11da/R-S11db/R-S11dc bootstrap-candidate refresh — SOURCE CORRECTION AND FOCUSED
  FIXTURE EVIDENCE PRESENT; REAL ACQUISITION-VM CANDIDATE, PROMOTION, CERTIFICATION, AND PRODUCT
  PRODUCERS OPEN.** Commit `4ea5ecbf2f632b617df901b2767fb584ddb2abd5` moved each role's build and
  private capture into one disposable acquisition-VM generation. Its first exact Android execution reached the
  real pinned-base package install but failed before capture: the acquisition Dockerfile required the historical
  final package-manifest SHA-256, so a refresh could not discover a changed repository closure. The failed run's
  before/during/after host-listener inventories were byte-identical, both new-listener sets were empty, and no
  QEMU or virtiofsd generation remained; no bootstrap candidate was published.

  The first exact VM run of commit `84d79bb61c93c13b61d27a5919b09724cd899b1f` then completed the
  current Ubuntu package installation and built the discovery image. Its required confined nonroot inspection
  refused before metadata sealing because the embedded Dockerfile inherited mode 0600 from the acquisition
  VM's intentionally private exact-commit checkout. Host listeners again remained byte-identical before,
  during, and after; both new-listener sets were empty, the exact VM processes were joined, and no candidate
  was published. Current source explicitly makes the embedded recipe, package manifest, and contract root-owned
  mode 0444 in the same install layer so nonroot inspection and later UID/GID-1000 certification receive read
  authority without write authority.

  The next exact run at `21db3821d3fb9802e8bddc5278c8b0ba2813aa20` passed that confined nonroot
  inspection and built the four-instruction networkless seal. Exact configuration equality then rejected an
  additional transformation by Docker's legacy builder before capture. The run again left byte-identical host
  listeners, empty new-listener sets, no owned process, and no candidate. The validator now reports the bounded
  exact differing configuration keys so the transformation can be observed and resolved without guessing or
  silently broadening the accepted candidate contract.

  The exact diagnostic run at `ee826671edb7dd2024ff36e4c250244764eeb70a` found only
  `Config.Image`: the discovery and candidate values were different SHA-256 image references, while rootfs and
  every other configuration field matched. Moby defines that key as the operator-supplied source-image name;
  OCI does not include it among image execution parameters. Current validation therefore removes exactly that
  key from both sides before comparing the complete runtime configuration, still permits only the one derived
  package label, and retains the rootfs/config mutation negatives. A clean exact-commit rerun remains required.

  Current source separates the authorities without changing any reviewed final pin. Each exact-commit
  acquisition Dockerfile installs once, writes the canonical sorted package inventory, derives its hash from
  those bytes, and embeds that value in the contract. It does not accept or label a preselected package hash.
  The shared four-instruction `Dockerfile.builder-bootstrap-seal` is then supplied through stdin with no
  filesystem context; it names the exact local discovery image ID and adds only the derived package label under
  no-pull, no-cache, fixed-platform, network-disabled execution. The executable validator runs the discovery
  image as confined nonroot without network, requires live/stored inventory and contract equality, and accepts
  the sealed candidate only when rootfs is byte-identical by layer identity and every OCI runtime configuration
  field except that one label is unchanged. Candidate capture reports the newly observed recipe and package identities beside
  the image/config/manifest/archive/size/layout proposals. Existing final pins still govern promotion and
  certification; neither a discovery nor a candidate enters the ordinary loader.

  The generic executable provenance self-test covers derived-contract success, stale contract rejection,
  premature package-label refusal, rootfs mutation refusal, and unrelated configuration mutation refusal. The
  three compact role gates bind the exact acquisition recipes, exact metadata-only seal, shared transaction,
  role wrapper, candidate output, final-only loaders, and separate verify-before-no-clobber promotion. This is
  source/fixture evidence only. A clean pushed commit must execute the real Android acquisition VM, produce and
  independently inspect the private candidate, prove listener/process/residue finality, then undergo deliberate
  pin review, bootstrap promotion, networkless UID/GID-1000 certification, and the real Gradle/Android producer
  before this prerequisite or any release artifact claim can close.
