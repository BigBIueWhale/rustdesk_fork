...
// R-X12 / R-G8 (§19): the Wayland-keyboard prompt machinery is excised. The fork is X11-pinned, so
// no peer is ever Wayland (pi.isWayland always false) — the prompt was dead — and it carried an
// upstream github "learn more" link (de-brand). The showWaylandKeyboardInputWarningDialog + these
// helpers + the url_launcher import are removed; keyboardInputAllowed defaults true so the keyboard
// is unaffected.
...
List<TTextMenu> toolbarControls(BuildContext context, String id, FFI ffi) {
  ...
  if (isDefaultConn &&
    ...
    v.add(TTextMenu(
        ...
        onPressed: () async {
          // R-X12/R-G8 (§19): the fork is X11-pinned, so no peer is Wayland (pi.isWayland is
          // always false) — the Wayland-keyboard prompt + its upstream-github "learn more" link
          // are excised. Send clipboard keystrokes directly (keyboardInputAllowed defaults true).
  ...
  if (pi.version.isNotEmpty) {
    v.add(TTextMenu(
      ...
      onPressed: () => sessionRefreshVideo(sessionId, ffi.clientOwnerId),
    ));
  }
  // record
  if (!(isDesktop || isWeb) &&
  ...
  if (pi.platform != kPeerPlatformAndroid &&
    ...
    v.add(TToggleMenu(
        ...
        onChanged: (selected) async {
          ...
          final enabled = bind.sessionGetToggleOptionSync(
              sessionId: sessionId, arg: option);
          showCursorLockState.value = enabled;
