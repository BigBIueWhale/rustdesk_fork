...
class _RemotePageState extends State<RemotePage>
  ...
  void initState() {
    ...
    // R-X12 (§19): the Wayland-keyboard-mode normalizer is excised — the X11-pinned fork has no
    // Wayland peers (pi.isWayland always false), so it was always a no-op.
  }
  ...
  Future<void> _prepareForRemoval({required bool closeSession}) {
    ...
    if (cleanup != null) {
      if (_cleanupClosesSession != closeSession) {
        debugPrint('REMOTE PAGE cleanup policy already fixed for ${widget.id}: '
  ...
  Future<void> _awaitCleanup(String operation, Future<dynamic> cleanup) async {
    try {
      ...
      debugPrint(
          'REMOTE PAGE $operation failed for ${widget.id}: ${error.runtimeType}');
  ...
  Future<void> _cleanupResources({required bool closeSession}) async {
    ...
    try {
      ...
      debugPrint(
          'REMOTE PAGE shared-state retirement failed for ${widget.id}: ${error.runtimeType}');
      debugPrintStack(stackTrace: stackTrace);
    }
  }
