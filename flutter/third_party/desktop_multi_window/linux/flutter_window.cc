...
namespace
{
  ...
  {
    std::shared_ptr<FlutterWindowCallback> callback;
    int64_t id;
  };

  gboolean destroyWindowWhenIdle(gpointer data)
  {
    auto *pending = static_cast<PendingWindowDestroy *>(data);
    pending->callback->OnWindowDestroy(pending->id);
    return G_SOURCE_REMOVE;
  }

  void deletePendingWindowDestroy(gpointer data)
  {
...
gboolean onWindowClose(GtkWidget *widget, GdkEvent *, gpointer arg)
{
  ...
  // The manager owns self. Erasing it synchronously from this callback destroys
  // self before the callback returns and used to read isPreventClose from freed
  // memory below. Keep GTK's default handler from destroying the widget and
  // retire the owning map entry after this signal emission has unwound.
