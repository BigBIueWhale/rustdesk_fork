...
static void fl_url_launcher_plugin_dispose(GObject* object) {
  ...
  // The two Pigeon handlers jointly retain one API object, and that API owns
  // this plugin. Consequently the plugin reaches its last reference only when
  // the messenger is already removing those handlers. Trying to clear them
  // again here recursively creates channels while the engine is shutting down.
