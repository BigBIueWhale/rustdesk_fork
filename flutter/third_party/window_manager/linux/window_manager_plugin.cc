...
static void window_manager_plugin_handle_method_call(
  ...
  // Platform messages can already be queued when GTK destroys the owning
  // window. A registrar can outlive its view, so deriving the toplevel from it
  // here used to return stale storage and crash in window-state queries.
...
static void window_manager_plugin_dispose(GObject* object) {
  ...
  if (self->button_press_emission_hook != 0) {
    g_signal_remove_emission_hook(
        g_signal_lookup("button-press-event", GTK_TYPE_WIDGET),
        self->button_press_emission_hook);
    self->button_press_emission_hook = 0;
  }
  self->_event_box = nullptr;
  self->grab_pointer = nullptr;
  g_clear_object(&self->css_provider);
  g_clear_pointer(&self->title_bar_style_, g_free);
...
void _emit_event(WindowManagerPlugin* plugin, const char* event_name) {
  if (plugin->window == nullptr || plugin->channel == nullptr) {
    return;
  }
  g_autoptr(FlValue) result_data = fl_value_new_map();
...
void on_window_destroy(GtkWidget* widget, gpointer data) {
  WindowManagerPlugin* plugin = WINDOW_MANAGER_PLUGIN(data);
  if (plugin->window != GTK_WINDOW(widget)) {
    return;
  }
  if (plugin->button_press_emission_hook != 0) {
    g_signal_remove_emission_hook(
        g_signal_lookup("button-press-event", GTK_TYPE_WIDGET),
        plugin->button_press_emission_hook);
    plugin->button_press_emission_hook = 0;
  }
  plugin->window = nullptr;
  plugin->_event_box = nullptr;
  plugin->grab_pointer = nullptr;
}
...
GObject* window_manager_plugin_register_with_registrar_for_window(
  ...
  if (window != nullptr) {
    // Disconnect the engine's legacy delete handler before installing the
    // plugin's close policy. See flutter/engine#40033.
    guint handler_id = g_signal_handler_find(
        window, G_SIGNAL_MATCH_DATA, 0, 0, NULL, NULL,
    ...
    g_signal_connect(window, "delete_event", G_CALLBACK(on_window_close),
                     plugin);
    g_signal_connect(window, "focus-in-event", G_CALLBACK(on_window_focus),
                     plugin);
    g_signal_connect(window, "focus-out-event", G_CALLBACK(on_window_blur),
                     plugin);
    g_signal_connect(window, "show", G_CALLBACK(on_window_show), plugin);
    g_signal_connect(window, "hide", G_CALLBACK(on_window_hide), plugin);
    g_signal_connect(window, "check-resize", G_CALLBACK(on_window_resize),
                     plugin);
    g_signal_connect(window, "configure-event", G_CALLBACK(on_window_move),
                     plugin);
  ...
  GObject* registered_plugin = G_OBJECT(plugin);
...
void window_manager_plugin_register_with_registrar(
  ...
  window_manager_plugin_register_with_registrar_for_window(registrar, window);
}
