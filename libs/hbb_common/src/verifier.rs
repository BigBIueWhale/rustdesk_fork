...
pub fn client_config(_danger_accept_invalid_cert: bool) -> ResultType<ClientConfig> {
    // R-G4/§8: keep the old boolean-shaped API for call-site compatibility, but the fork never
    // installs a verifier that accepts an invalid certificate.
    client_config_safe()
}
