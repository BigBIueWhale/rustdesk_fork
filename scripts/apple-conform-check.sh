...
grep -q 'BeginMacosServiceOwnedUnattendedPasswordChange(String)' "$REPO/src/ipc.rs" || r_s11b2="$r_s11b2 macos-service-password-begin-does-not-carry-value"
grep -q 'password: Option<String>' "$REPO/src/ipc.rs" || r_s11b2="$r_s11b2 macos-service-password-pending-value-missing"
grep -q 'fn take_password(&mut self) -> ResultType<String>' "$REPO/src/ipc.rs" || r_s11b2="$r_s11b2 macos-service-password-pending-value-not-take-only"
grep -q 'impl Drop for MacosServiceOwnedPasswordRequest' "$REPO/src/ipc.rs" || r_s11b2="$r_s11b2 macos-service-password-pending-value-not-zeroed"
...
grep -q 'fn macos_schedule_service_owned_password_request_expiry' "$REPO/src/ipc.rs" || r_s11b2="$r_s11b2 macos-service-password-expiry-task-missing"
...
grep -Fq 'fn macos_installed_app_is_expected_and_trusted(peer_exe: &Path) -> bool' "$REPO/src/ipc/auth.rs" || r_s11b2="$r_s11b2 macos-service-ipc-app-trust-missing"
...
grep -Fq 'const MACOS_LS: &str = "/bin/ls";' "$REPO/src/ipc/auth.rs" || r_s11c5="$r_s11c5 macos-ls-absolute-missing"
...
grep -Fq 'fn macos_installed_app_is_expected_and_trusted(peer_exe: &Path) -> bool' "$REPO/src/ipc/auth.rs" || r_s11c5="$r_s11c5 macos-service-ipc-app-trust-missing"
...
grep -Fq 'Command::new(MACOS_LS)' "$REPO/src/ipc/auth.rs" || r_s11c5="$r_s11c5 macos-runtime-acl-check-not-absolute"
...
macos_app_trust_block=$(awk '/fn macos_installed_app_is_expected_and_trusted/,/^}/' "$REPO/src/ipc/auth.rs")
...
line_app_check=$(grep -n 'macos_installed_app_is_expected_and_trusted(peer_exe)' "$REPO/src/ipc/auth.rs" | tail -n 1 | cut -d: -f1)
line_helper_check=$(grep -n 'macos_privileged_helper_is_expected_and_trusted(current_exe)' "$REPO/src/ipc/auth.rs" | tail -n 1 | cut -d: -f1)
