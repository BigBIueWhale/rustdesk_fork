...
def analyze(sources):
    ...
    try:
        ...
        need("b1", "macos-service-snapshot-client-not-typed-receiver", all(token in snapshot_client for token in [
            "MacosServiceOwnedCredentialReplicaReceiver::connect(deadline).await?",
            "receiver.receive_and_admit(deadline).await?",
            "admission.install()",
        ]) and not any(token in snapshot_client for token in [
            "Endpoint::connect(", "macos_service_server_authorization_snapshot(",
    ...
    try:
        ...
        user_owned_password_begin = item(ipc, "fn begin_user_owned_password_mutation")
        ...
        mac_credential_requester_generation = item(auth, "pub(crate) fn macos_service_owned_credential_requester_identity_is_live")
        mac_credential_post_request = item(auth, "pub(crate) fn macos_service_owned_credential_requester_matches_post_request_authorization")
        ...
        mac_password_right_admission_type = item(ipc, "struct MacosServiceOwnedPasswordRightAdmission")
        mac_password_right_admission_grant = item(ipc, "fn grant_macos_service_owned_password_right_admission")
        mac_password_right_admission_action = item(ipc, "impl MacosServiceOwnedPasswordRightAdmission")
        ...
        snapshot_peer = item(ipc, "async fn authenticate_macos_service_owned_credential_requester")
        snapshot_identity = item(ipc, "fn authenticate_macos_service_owned_credential_requester_blocking")
        snapshot_final = item(ipc, "fn macos_service_owned_credential_requester_is_live")
        ...
        mac_service_server_task_proof = item(ipc, "async fn authorize_macos_service_server_snapshot_for_task")
        ...
        linux_runtime_prs_receiver = item(ipc, "impl LinuxServiceOwnedPasswordReplicaReceiver")
        sensitive_main_authority = item(ipc, "fn sensitive_main_ipc_authority")
        sensitive_main_authority_kind = item(ipc, "impl SensitiveMainPasswordAuthority")
        linux_runtime_prs_prepare = item(ipc, "fn prepare_linux_service_owned_runtime_prs")
        linux_runtime_prs_begin = item(ipc, "fn begin_linux_service_owned_runtime_prs_mutation")
        generic_password_worker = item(ipc, "fn spawn_password_mutation")
        linux_runtime_prs_worker = item(ipc, "fn spawn_linux_service_owned_runtime_prs_mutation")
        linux_runtime_prs_parent = item(auth, "fn authenticate_linux_service_owned_password_parent")
        ...
        need("b2", "main-peer-auth-not-before-secret-read", ordered(run_main, [
            ...
            "try_acquire_sensitive_main_ipc_transaction_slot(", "authority.mutation_kind()",
            "handle_sensitive_main_ipc_transaction(",
        ...
        need("b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit",
            ...
            and "ServiceOwnedRuntimePrs(LinuxServiceOwnedPasswordReplicaReceiver)" in ipc
            ...
            and "#[derive(Copy)]\nenum SensitiveMainPasswordAuthority" not in ipc
            and ordered(sensitive_main_authority_kind, [
            ...
            and ordered(linux_runtime_prs_receiver, [
                ...
                "!crate::common::is_service_owned_server_process()",
                "authenticate_linux_service_owned_password_parent(", "stream",
                "password::USER_PASSWORD_IPC_POSTFIX", "Ok(Self { parent })",
                "fn admit<T>(self, stream: &T) -> ResultType<LinuxServiceOwnedRuntimePrsAdmission>",
                "!crate::common::is_service_owned_server_process()",
                "authenticate_linux_service_owned_password_parent(", "stream",
                "password::USER_PASSWORD_IPC_POSTFIX", "if refreshed != self.parent",
            ...
            and ordered(linux_runtime_prs_parent, [
                "-> ResultType<LinuxProcessIdentity>", "peer_uid_from_fd(fd)", "peer_uid != 0",
                "peer_pid_from_fd(fd)", "linux_kernel_process_identity_by_pid(peer_pid)?",
                "identity.uid != peer_uid", "SERVICE_OWNED_SERVER_LAUNCH_PARENT_ENV",
                "linux_proc_parent_pid(std::process::id())?",
                "peer_pid != expected_parent || actual_parent != expected_parent", "Ok(identity)",
            ])
            and "pub(crate) fn authenticate_linux_service_owned_password_parent" not in auth
            and ordered(linux_runtime_prs_prepare, [
                "_admission: LinuxServiceOwnedRuntimePrsAdmission", "operation_id: &str",
                "replica: &ServiceOwnedRuntimePrsReplica", "PasswordMutationKind::ServiceOwned",
            ...
            and ordered(main_sensitive, [
                "authority: SensitiveMainPasswordAuthority", "password::receive_request_unix(",
                "request.into_password()", "match authority",
                "SensitiveMainPasswordAuthority::UserOwned", "begin_user_owned_password_mutation(",
                "SensitiveMainPasswordAuthority::ServiceOwnedRuntimePrs(receiver)",
                "receiver.admit(&stream)", "begin_linux_service_owned_runtime_prs_mutation(",
                "ServiceOwnedRuntimePrsReplica { value }", "password::send_status_unix(", "worker.await",
            ])
            and "begin_password_mutation(" not in main_sensitive
            and "authenticate_linux_service_owned_password_parent" not in main_sensitive
            and "set_permanent_password_prs_for_runtime" not in main_sensitive
            and ordered(linux_runtime_prs_begin, [
                "admission: LinuxServiceOwnedRuntimePrsAdmission", "operation_id: String",
                "replica: ServiceOwnedRuntimePrsReplica", "PasswordMutationKind::ServiceOwned",
                "prepare_linux_service_owned_runtime_prs(", "admission", "&operation_id", "&replica",
                "try_acquire_main_ipc_blocking_mutation_slot()", "acknowledge(&operation_id, kind, value)",
            ...
            and ordered(linux_runtime_prs_worker, [
                "operation_id: String", "replica: ServiceOwnedRuntimePrsReplica",
                "permit: OwnedSemaphorePermit", "tokio::task::spawn_blocking(move ||",
                "PasswordMutationKind::ServiceOwned", "replica.install_for_runtime()",
        ...
        need("b2", "windows-password-authority-not-typed-through-queue-and-ledger-admission",
            ...
            and "begin_user_owned_password_mutation(operation_id, value, authority_allowed)" in windows_user_password_begin
            and "let kind = PasswordMutationKind::UserOwned;" in user_owned_password_begin
        ...
        need("b2", "macos-password-right-policy-write-requester-authority-not-exact",
             ...
             ]) and all(token not in readiness_server for token in [
             ...
             ]) and "requester: MacosServiceOwnedPasswordRequester" in mac_password_right_admission_type
             and "#[derive(Clone)]\nstruct MacosServiceOwnedPasswordRightAdmission" not in ipc
             and "#[derive(Copy)]\nstruct MacosServiceOwnedPasswordRightAdmission" not in ipc
             and ordered(mac_password_right_admission_grant, [
                 "requester: MacosServiceOwnedPasswordRequester",
             ...
             ]) and "bool" not in mac_password_right_admission_grant
             and ipc.count("Some(MacosServiceOwnedPasswordRightAdmission { requester })") == 1
             and ordered(mac_password_right_admission_action, [
                 "fn ensure_ready(self) -> bool",
                 "macos_service_owned_password_requester_is_live(&self.requester)",
                 "&& crate::platform::ensure_service_owned_unattended_password_authorization_right()",
             ]) and "macos_service_owned_password_requester_is_live(&self.requester)\n            && crate::platform::ensure_service_owned_unattended_password_authorization_right()" in mac_password_right_admission_action)
        ...
        proof_callers = readiness_server + mac_password_right_admission_grant + mac_password_right_admission_action + snapshot_peer + snapshot_client + mac_password_auth + mac_credential_auth + mac_service_auth + mac_sensitive + connect_sensitive + connect_service
        ...
        need("b2", "readiness-snapshot-not-password-budgeted", ordered(readiness_server, [
            "try_acquire_macos_service_password_ipc_authorization_slot()", "run_bounded_macos_security_proof(deadline", "grant_macos_service_owned_password_right_admission(", "admission.ensure_ready()",
        ]) and "ensure_service_owned_unattended_password_authorization_right()" in mac_password_right_admission_action and ordered(snapshot_peer, [
            "try_acquire_macos_service_credential_ipc_authorization_slot()",
        ...
        need("b2", "macos-password-verification-mutates-authorization-policy",
             "RustDeskSetUnattendedPasswordRightMatchesExpected()" in native_create
             and native_verify.count("RustDeskSetUnattendedPasswordRightMatchesExpected()") == 2
             and "EnsureRustDeskSetUnattendedPasswordRight()" not in native_verify
             and "AuthorizationRightSet(" not in native_verify
             and native_verify.count("AuthorizationFree(") == 1
             and "AuthorizationFree(authRef, kAuthorizationFlagDefaults)" not in native_verify
             and "AuthorizationRightSet(" in right_write
        ...
        ]) and "macos_service_owned_server_live_argv_is_expected(&argv)" in snapshot_identity
            and "macos_service_owned_server_live_argv_is_expected(process.cmd())" in snapshot_final)
        need("b2", "snapshot-requester-not-installed-launchd-plist-proven", ordered(snapshot_identity, [
        ...
        ]) and ordered(mac_credential_requester_auth, [
            ...
            "macos_service_owned_password_requester_identity_is_live(&identity)",
            "Ok(identity)",
        ]) and ordered(mac_credential_requester_generation, [
            "macos_service_owned_password_requester_identity_is_live(identity)",
            "&& macos_service_owned_password_requester_generation_is_live(identity)",
        ]) and ordered(snapshot_final, [
        ...
        need("b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install",
            ...
            and ordered(mac_service_server_authorization, [
                "identity: MacosPeerProcessIdentity", "context: &'static str",
            ])
            and ordered(mac_service_server_verify, [
            ...
            and ordered(mac_service_server_match, [
            ...
            and ordered(mac_runtime_prs_receiver, [
                ...
                "!crate::common::is_service_owned_server_process()",
                "hbb_common::uuid::Uuid::new_v4()",
                "password::send_credential_snapshot_request_unix(", "&mut self.stream", "operation_id",
                "password::receive_credential_replica_unix(", "&mut self.stream", "operation_id",
                "ipc_auth::macos_service_server_authorization_snapshot(", "&self.stream",
                "let refreshed =",
                ...
                "MacosServiceOwnedRuntimePrsAdmission {", "_receiver: self",
                "replica: ServiceOwnedRuntimePrsReplica { value }",
            ])
            ...
            and ordered(mac_runtime_prs_admission, [
                "fn install(self) -> ResultType<bool>", "self.replica.install_for_runtime()",
            ])
            ...
            and not any(token in mac_runtime_prs_client for token in [
                "Endpoint::connect(", "macos_service_server_authorization_snapshot(",
                "send_credential_snapshot_request_unix(", "receive_credential_replica_unix(",
                "Config::set_permanent_password_prs_for_runtime",
            ])
...
mutation("macos-password-right-admission-clone", "ipc", "struct MacosServiceOwnedPasswordRightAdmission {", "#[derive(Clone)]\nstruct MacosServiceOwnedPasswordRightAdmission {", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-admission-requester", "ipc", "struct MacosServiceOwnedPasswordRightAdmission", "requester: MacosServiceOwnedPasswordRequester,", "requester_authorized: bool,", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-admission-borrow", "ipc", "fn grant_macos_service_owned_password_right_admission", "requester: MacosServiceOwnedPasswordRequester,", "requester: &MacosServiceOwnedPasswordRequester,", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-admission-match", "ipc", "fn grant_macos_service_owned_password_right_admission", "if !macos_service_owned_password_right_requester_matches_post_request_authorization(", "if false && !macos_service_owned_password_right_requester_matches_post_request_authorization(", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-action-consume", "ipc", "impl MacosServiceOwnedPasswordRightAdmission", "fn ensure_ready(self) -> bool", "fn ensure_ready(&self) -> bool", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-final-replay", "ipc", "impl MacosServiceOwnedPasswordRightAdmission", "macos_service_owned_password_requester_is_live(&self.requester)", "true", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-policy-disjunction", "ipc", "impl MacosServiceOwnedPasswordRightAdmission", "&& crate::platform::ensure_service_owned_unattended_password_authorization_right()", "|| crate::platform::ensure_service_owned_unattended_password_authorization_right()", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-grant-bypass", "ipc", "fn macos_service_owned_password_authorization_right_is_ready", "grant_macos_service_owned_password_right_admission(", "grant_macos_service_owned_password_right_admission_disabled(", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
scoped_mutation("macos-password-right-action-bypass", "ipc", "fn macos_service_owned_password_authorization_right_is_ready", "Ok(admission.ensure_ready())", "Ok(crate::platform::ensure_service_owned_unattended_password_authorization_right())", "b2", "macos-password-right-policy-write-requester-authority-not-exact")
...
scoped_mutation("linux-runtime-prs-install-consume", "ipc", "impl ServiceOwnedRuntimePrsReplica", "fn install_for_runtime(self) -> ResultType<bool>", "fn install_for_runtime(&self) -> ResultType<bool>", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
mutation("linux-runtime-prs-receiver-identity", "ipc", "struct LinuxServiceOwnedPasswordReplicaReceiver {\n    parent: LinuxProcessIdentity,\n}", "struct LinuxServiceOwnedPasswordReplicaReceiver {\n    parent_alive: bool,\n}", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
mutation("linux-runtime-prs-receiver-clone", "ipc", "struct LinuxServiceOwnedPasswordReplicaReceiver {", "#[derive(Clone)]\nstruct LinuxServiceOwnedPasswordReplicaReceiver {", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
mutation("linux-runtime-prs-receiver-public", "ipc", "struct LinuxServiceOwnedPasswordReplicaReceiver {", "pub struct LinuxServiceOwnedPasswordReplicaReceiver {", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
mutation("linux-runtime-prs-admission-clone", "ipc", "struct LinuxServiceOwnedRuntimePrsAdmission {", "#[derive(Clone)]\nstruct LinuxServiceOwnedRuntimePrsAdmission {", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
mutation("linux-runtime-prs-action-payload", "ipc", "ServiceOwnedRuntimePrs(LinuxServiceOwnedPasswordReplicaReceiver),", "ServiceOwnedRuntimePrs(PasswordMutationKind),", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-action-kind", "ipc", "impl SensitiveMainPasswordAuthority", "Self::ServiceOwnedRuntimePrs(_) => PasswordMutationKind::ServiceOwned,", "Self::ServiceOwnedRuntimePrs(_) => PasswordMutationKind::UserOwned,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-receiver-role", "ipc", "impl LinuxServiceOwnedPasswordReplicaReceiver", "if !crate::common::is_service_owned_server_process() {\n            bail!(\n                \"Linux service-owned password replica receiver requires the exact service-owned server role\"", "if false && !crate::common::is_service_owned_server_process() {\n            bail!(\n                \"Linux service-owned password replica receiver requires the exact service-owned server role\"", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-receiver-endpoint", "ipc", "impl LinuxServiceOwnedPasswordReplicaReceiver", "let parent = authenticate_linux_service_owned_password_parent(\n            stream,\n            password::USER_PASSWORD_IPC_POSTFIX,", "let parent = authenticate_linux_service_owned_password_parent(\n            stream,\n            password::SERVICE_CREDENTIAL_IPC_POSTFIX,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-admission-consume", "ipc", "impl LinuxServiceOwnedPasswordReplicaReceiver", "fn admit<T>(self, stream: &T)", "fn admit<T>(&self, stream: &T)", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-admission-role", "ipc", "impl LinuxServiceOwnedPasswordReplicaReceiver", "if !crate::common::is_service_owned_server_process() {\n            bail!(\n                \"Linux service-owned password replica receiver lost its exact service-owned server role\"", "if false && !crate::common::is_service_owned_server_process() {\n            bail!(\n                \"Linux service-owned password replica receiver lost its exact service-owned server role\"", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-admission-endpoint", "ipc", "impl LinuxServiceOwnedPasswordReplicaReceiver", "let refreshed = authenticate_linux_service_owned_password_parent(\n            stream,\n            password::USER_PASSWORD_IPC_POSTFIX,", "let refreshed = authenticate_linux_service_owned_password_parent(\n            stream,\n            password::SERVICE_CREDENTIAL_IPC_POSTFIX,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-parent-continuity", "ipc", "impl LinuxServiceOwnedPasswordReplicaReceiver", "if refreshed != self.parent {", "if false && refreshed != self.parent {", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-parent-generation", "auth", "pub(super) fn authenticate_linux_service_owned_password_parent", "let identity = linux_kernel_process_identity_by_pid(peer_pid)?;", "let identity = LinuxProcessIdentity { pid: peer_pid, uid: peer_uid, start_time: String::new() };", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
mutation("linux-runtime-prs-parent-visibility", "auth", "pub(super) fn authenticate_linux_service_owned_password_parent<T>(", "pub(crate) fn authenticate_linux_service_owned_password_parent<T>(", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-ledger-consume", "ipc", "fn prepare_linux_service_owned_runtime_prs", "_admission: LinuxServiceOwnedRuntimePrsAdmission,", "_admission: &LinuxServiceOwnedRuntimePrsAdmission,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-ledger-type", "ipc", "fn prepare_linux_service_owned_runtime_prs", "replica: &ServiceOwnedRuntimePrsReplica,", "replica: &SensitivePassword,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-ledger-kind", "ipc", "fn prepare_linux_service_owned_runtime_prs", "PasswordMutationKind::ServiceOwned,", "PasswordMutationKind::UserOwned,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-handler-admission", "ipc", "async fn handle_sensitive_main_ipc_transaction", "match receiver.admit(&stream) {", "match Ok(LinuxServiceOwnedRuntimePrsAdmission { _receiver: receiver }) {", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-entry-boolean", "ipc", "fn begin_linux_service_owned_runtime_prs_mutation", "admission: LinuxServiceOwnedRuntimePrsAdmission,", "admission: bool,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-worker-type", "ipc", "fn spawn_linux_service_owned_runtime_prs_mutation", "replica: ServiceOwnedRuntimePrsReplica,", "replica: SensitivePassword,", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
scoped_mutation("linux-runtime-prs-worker-persistence", "ipc", "fn spawn_linux_service_owned_runtime_prs_mutation", "let result = match replica.install_for_runtime() {", "let result = match Config::set_permanent_password_persisted(replica.as_sensitive_password().as_str()) {", "b2", "linux-runtime-prs-receiver-authority-not-typed-through-final-commit")
...
scoped_mutation("macos-password-admission-policy-write", "ipc", "fn grant_macos_service_owned_password_admission", "if !crate::platform::verify_service_owned_unattended_password_authorization(authorization) {", "if !crate::platform::ensure_service_owned_unattended_password_authorization_right() { return None; }\n    if !crate::platform::verify_service_owned_unattended_password_authorization(authorization) {", "b2", "macos-password-verification-mutates-authorization-policy")
...
scoped_mutation("macos-runtime-prs-server-authorization-return", "auth", "pub(crate) fn authorize_macos_service_server_snapshot", ") -> ResultType<MacosServiceServerAuthorization>", ") -> ResultType<()>", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-server-uid-continuity", "auth", "pub(crate) fn macos_service_server_authorizations_match", "accepted.identity.uid == refreshed.identity.uid", "true", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-server-pid-continuity", "auth", "pub(crate) fn macos_service_server_authorizations_match", "&& accepted.identity.pid == refreshed.identity.pid", "&& true", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-server-token-continuity", "auth", "pub(crate) fn macos_service_server_authorizations_match", "&& accepted.identity.audit_token == refreshed.identity.audit_token", "&& true", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-initial-role", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "if !crate::common::is_service_owned_server_process() {\n            bail!(\"macOS service credential snapshots require the exact service-owned server role\")", "if false && !crate::common::is_service_owned_server_process() {\n            bail!(\"macOS service credential snapshots require the exact service-owned server role\")", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-fixed-endpoint", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "Config::ipc_path_for_uid(0, password::SERVICE_CREDENTIAL_IPC_POSTFIX)", "Config::ipc_path_for_uid(0, password::USER_PASSWORD_IPC_POSTFIX)", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-initial-proof-retention", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "let server =\n            authorize_macos_service_server_snapshot_for_task(authorization, deadline).await?", "let _server =\n            authorize_macos_service_server_snapshot_for_task(authorization, deadline).await?", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-receiver-consume", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "async fn receive_and_admit(\n        mut self,", "async fn receive_and_admit(\n        &mut self,", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-final-role", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "if !crate::common::is_service_owned_server_process() {\n            bail!(\"macOS service credential receiver lost the exact service-owned server role\");", "if false && !crate::common::is_service_owned_server_process() {\n            bail!(\"macOS service credential receiver lost the exact service-owned server role\");", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-raw-response", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "password::receive_credential_replica_unix(", "password::receive_request_unix(", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-final-snapshot", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "let refreshed = ipc_auth::macos_service_server_authorization_snapshot(", "let refreshed = ipc_auth::macos_service_server_authorization_snapshot_disabled(", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-final-proof", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "authorize_macos_service_server_snapshot_for_task(refreshed, deadline).await?", "refreshed", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-final-continuity", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "if !ipc_auth::macos_service_server_authorizations_match(&self.server, &refreshed) {", "if false {", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
mutation("macos-runtime-prs-admission-receiver", "ipc", "    _receiver: MacosServiceOwnedCredentialReplicaReceiver,", "    helper_authorized: bool,", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
mutation("macos-runtime-prs-admission-type", "ipc", "    replica: ServiceOwnedRuntimePrsReplica,\n}\n\n#[cfg(any(target_os = \"linux\", target_os = \"macos\"))]", "    replica: SensitivePassword,\n}\n\n#[cfg(any(target_os = \"linux\", target_os = \"macos\"))]", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-admission-construction", "ipc", "impl MacosServiceOwnedCredentialReplicaReceiver", "Ok(MacosServiceOwnedRuntimePrsAdmission {", "Ok(MacosServiceOwnedRuntimePrsAdmissionDisabled {", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-install-consume", "ipc", "impl MacosServiceOwnedRuntimePrsAdmission", "fn install(self) -> ResultType<bool>", "fn install(&self) -> ResultType<bool>", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-install-sink", "ipc", "impl MacosServiceOwnedRuntimePrsAdmission", "self.replica.install_for_runtime()", "Config::set_permanent_password_prs_for_runtime(self.replica.as_sensitive_password().as_str())", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-cross-unix-sink", "ipc", "impl ServiceOwnedRuntimePrsReplica", "fn install_for_runtime(self) -> ResultType<bool>", "#[cfg(target_os = \"linux\")]\n    fn install_for_runtime(self) -> ResultType<bool>", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
scoped_mutation("macos-runtime-prs-client-bypass", "ipc", "pub async fn refresh_macos_service_owned_permanent_password_snapshot", "let receiver = MacosServiceOwnedCredentialReplicaReceiver::connect(deadline).await?", "let receiver = connect_service(1_000).await?", "b2", "macos-runtime-prs-receiver-authority-not-typed-through-final-install")
...
scoped_mutation("native-password-verification-free-result", "macos_mm", 'extern "C" bool MacVerifyServiceOwnedUnattendedPasswordAuthorizationExternalForm', "OSStatus freeStatus = AuthorizationFree(authRef, kAuthorizationFlagDestroyRights);", "AuthorizationFree(authRef, kAuthorizationFlagDestroyRights);\n    OSStatus freeStatus = errAuthorizationSuccess;", "b2", "macos-password-verification-mutates-authorization-policy")
scoped_mutation("native-password-verification-final-policy-order", "macos_mm", 'extern "C" bool MacVerifyServiceOwnedUnattendedPasswordAuthorizationExternalForm', "OSStatus freeStatus = AuthorizationFree(authRef, kAuthorizationFlagDestroyRights);\n    bool rightStillMatches = RustDeskSetUnattendedPasswordRightMatchesExpected();", "bool rightStillMatches = RustDeskSetUnattendedPasswordRightMatchesExpected();\n    OSStatus freeStatus = AuthorizationFree(authRef, kAuthorizationFlagDestroyRights);", "b2", "macos-password-verification-mutates-authorization-policy")
scoped_mutation("native-password-verification-final-policy-result", "macos_mm", 'extern "C" bool MacVerifyServiceOwnedUnattendedPasswordAuthorizationExternalForm', "return status == errAuthorizationSuccess &&\n           freeStatus == errAuthorizationSuccess &&\n           rightStillMatches;", "return status == errAuthorizationSuccess &&\n           freeStatus == errAuthorizationSuccess;", "b2", "macos-password-verification-mutates-authorization-policy")
scoped_mutation("native-password-verification-free-result-ignored", "macos_mm", 'extern "C" bool MacVerifyServiceOwnedUnattendedPasswordAuthorizationExternalForm', "return status == errAuthorizationSuccess &&\n           freeStatus == errAuthorizationSuccess &&\n           rightStillMatches;", "return status == errAuthorizationSuccess &&\n           rightStillMatches;", "b2", "macos-password-verification-mutates-authorization-policy")
scoped_mutation("native-password-verification-policy-disjunction", "macos_mm", 'extern "C" bool MacVerifyServiceOwnedUnattendedPasswordAuthorizationExternalForm', "return status == errAuthorizationSuccess &&\n           freeStatus == errAuthorizationSuccess &&\n           rightStillMatches;", "return status == errAuthorizationSuccess ||\n           freeStatus == errAuthorizationSuccess ||\n           rightStillMatches;", "b2", "macos-password-verification-mutates-authorization-policy")
...
grep -Fq 'The same identity additionally binds R-S11ii and Appendix C #394.' "$REPO/docs/NATIVE-CODEC-WATCH.md" || r_s11b2="$r_s11b2 linux-runtime-prs-typed-receiver-digest-binding-missing"
grep -Fq 'non-<code>Clone</code>, non-<code>Copy</code> <code>LinuxServiceOwnedPasswordReplicaReceiver</code>' "$REPO/requirements.html" || r_s11b2="$r_s11b2 linux-runtime-prs-receiver-capability-norm-missing"
grep -Fq 'consume that receiver to mint one <code>LinuxServiceOwnedRuntimePrsAdmission</code>' "$REPO/requirements.html" || r_s11b2="$r_s11b2 linux-runtime-prs-consuming-admission-norm-missing"
grep -Fq 'generic durable-password worker <span class="kw">MUST NOT</span> infer runtime-PRS behavior' "$REPO/requirements.html" || r_s11b2="$r_s11b2 linux-runtime-prs-worker-separation-norm-missing"
...
grep -Fq 'The same identity additionally binds R-S11ij and Appendix C #395.' "$REPO/docs/NATIVE-CODEC-WATCH.md" || r_s11b2="$r_s11b2 macos-runtime-prs-typed-receiver-digest-binding-missing"
...
grep -Fq 'The same identity additionally binds R-S11im and Appendix C #398.' "$REPO/docs/NATIVE-CODEC-WATCH.md" || r_s11b2="$r_s11b2 macos-password-right-typed-action-digest-binding-missing"
...
grep -Fq 'The same identity additionally binds R-S11in and Appendix C #399.' "$REPO/docs/NATIVE-CODEC-WATCH.md" || r_s11b2="$r_s11b2 macos-password-verification-read-only-digest-binding-missing"
