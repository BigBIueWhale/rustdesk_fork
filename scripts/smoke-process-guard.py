...
def server_identity_matches(pid, expected_start, expected_metadata):
    identity = read_process_identity(pid)
    if identity is None:
        return False
    state, start = identity
    ...
    return argv == [NEUTRAL_ARGV0, SERVER_ROLE]
...
def wait_server(pid, expected_start, executable_path):
    ...
    deadline = time.monotonic() + READY_WAIT_SECONDS
    while time.monotonic() < deadline:
        if server_identity_matches(pid, expected_start, expected):
            print(
