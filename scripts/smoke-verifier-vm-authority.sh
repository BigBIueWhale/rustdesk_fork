...
for source in "$OUTER_SOURCE" "$GUEST_SCRIPT" "$ENTRY_PREFLIGHT" "$VERIFY_SCRIPT" "$VERIFY_SCAN_SOURCE" "$FRB_CODEGEN_SOURCE" \
    ...
    "$ANDROID_BUILDER_IMAGE_CHECKER" "$ANDROID_BUILDER_CERTIFICATION_DOCKERFILE" \
...
/usr/bin/grep -Fq \
    'VERIFIER_VM_OFFLINE_IMAGE_PROVENANCE=pass uid=4000 gid=4000 android_decisions=39' \
    "$SERIAL_LOG" \
