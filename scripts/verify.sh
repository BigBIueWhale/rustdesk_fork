...
if grep -rqE '\("(empty_lan_tip|connecting_status|not_ready_status|ID/Relay Server|ID Server|Relay Server|Relay Connection|API Server|setup_server_tip|Use WebSocket)",' src/lang/; then
  echo "  FAIL §19: a removed dead rendezvous/relay/lan/WS lang key was re-added to a src/lang/ table"; rc=1
else
  echo "  ok  §19 dead rendezvous/relay/lan/WS lang keys absent (10: empty_lan_tip + status + ID/Relay/API-server cluster + setup_server_tip + Use-WebSocket)"
fi
