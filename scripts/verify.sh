...
if python3 scripts/verify-android-build-source.py --self-test \
  ...
  echo "  ok  R-S11e-76/R-S11e-77/R-S11e-78/R-S11e-79 Android APK builds use canonical-mode private exact-commit source, phased bounded scratch/preferences, narrow mounts, and confined existing-image launches"
else
  echo "  FAIL R-S11e-76/R-S11e-77/R-S11e-78/R-S11e-79 Android APK builder regained live-tree, archive-mode, output-tree, pull, network, root, ambient container/preferences, or simultaneous scratch-retention authority"
  rc=1
fi
...
r_s11c20=
unix_terminal_shell_block=$(awk '/fn get_default_shell\(\) -> Result<String>/,/^#\[cfg\(target_os = "macos"\)\]/' src/server/terminal_service.rs)
grep -Fq 'fn trusted_unix_terminal_shell_path(path: &Path) -> Option<PathBuf>' src/server/terminal_service.rs || r_s11c20="$r_s11c20 trusted-resolver-missing"
grep -Fq 'const UNIX_TERMINAL_SHELLS' src/server/terminal_service.rs || r_s11c20="$r_s11c20 candidate-set-missing"
grep -Fq 'metadata.uid() == 0' src/server/terminal_service.rs || r_s11c20="$r_s11c20 not-root-owned-gated"
grep -Fq 'metadata.mode() & 0o022 == 0' src/server/terminal_service.rs || r_s11c20="$r_s11c20 writable-mode-not-gated"
grep -Fq 'metadata.mode() & 0o111 != 0' src/server/terminal_service.rs || r_s11c20="$r_s11c20 executable-mode-not-gated"
grep -Fq 'trusted_unix_terminal_shell_rejects_relative_and_parent_paths' src/server/terminal_service.rs || r_s11c20="$r_s11c20 bad-path-test-missing"
grep -Fq 'trusted_unix_terminal_shell_returns_absolute_candidate_when_available' src/server/terminal_service.rs || r_s11c20="$r_s11c20 candidate-test-missing"
...
# (3b-iii-d9c6) R-S11ag/R-S11e-47: the macOS LaunchDaemon helper
# role is UID-0-only receiver authority. Account-name presentation cannot
...
r_s11e61=
python3 scripts/verify-macos-helper-build-binding.py --repo . \
...
grep -Fq 'MacosCodeSigningFlags::STRICT_VALIDATE' src/ipc/auth.rs || r_s11c5="$r_s11c5 macos-code-requirement-not-strict"
grep -Fq 'MacosCodeSigningFlags::CHECK_ALL_ARCHITECTURES' src/ipc/auth.rs || r_s11c5="$r_s11c5 macos-static-code-all-architectures-missing"
grep -Fq 'MacosCodeSigningFlags::CHECK_NESTED_CODE' src/ipc/auth.rs || r_s11c5="$r_s11c5 macos-static-app-nested-code-check-missing"
grep -Fq 'code.check_validity(validation_flags, &requirement)' src/ipc/auth.rs || r_s11c5="$r_s11c5 macos-static-code-validation-flags-not-used"
...
paste_task_rs=libs/clipboard/src/platform/unix/macos/paste_task.rs
r_s11e12=
...
if [ -n "$r_s11e12" ]; then
  ...
  echo "  ok  R-S11e-12 macOS clipboard-file paste uses fd-relative no-follow create/unlink/xattr/finalize with no path-based write fallback"
fi

echo "== (3c-iii-a1) macOS clipboard placeholder temp authority (R-S11e-13) =="
pasteboard_context_rs=libs/clipboard/src/platform/unix/macos/pasteboard_context.rs
item_data_provider_rs=libs/clipboard/src/platform/unix/macos/item_data_provider.rs
paste_observer_rs=libs/clipboard/src/platform/unix/macos/paste_observer.rs
pasteboard_readme=libs/clipboard/src/platform/unix/macos/README.md
r_s11e13=
...
if [ -n "$r_s11e13" ]; then
  ...
  echo "  ok  R-S11e-13 macOS clipboard-file paste placeholders use a private per-context temp dir with fd-relative exclusive create/unlink"
fi
...
# zbus pipewire surface is no longer linked). Asserts the feature enabling, `mod wayland`, directory,
# and both formerly orphaned source files are absent; uncompiled compatibility source is not accepted.
r_x12_cap=
...
if [ -e libs/scrap/src/wayland ] || [ -L libs/scrap/src/wayland ]; then
  r_x12_cap="$r_x12_cap scrap-wayland-dir"
fi
if [ -e libs/scrap/src/wayland.rs ] || [ -L libs/scrap/src/wayland.rs ]; then
  r_x12_cap="$r_x12_cap scrap-wayland-root-source"
fi
if [ -e libs/scrap/src/common/wayland.rs ] || [ -L libs/scrap/src/common/wayland.rs ]; then
  r_x12_cap="$r_x12_cap scrap-wayland-common-source"
fi
if [ -n "$r_x12_cap" ]; then
  echo "  FAIL R-X12: Wayland capture not compiled out:$r_x12_cap"; rc=1
else
  echo "  ok  R-X12/R-X12a Wayland/pipewire capture feature, module declarations, directory, and orphaned source files absent"
fi
...
# R-R2/R-R2c: one executable mobile build authority. The release scripts own the
# networked cache and networkless Android container; Flutter contributes only the exact
# arm64 cargo-ndk helper they invoke. Imported host runners, alternate ABI/package
# builders, F-Droid, and iOS helpers are deleted, not left as unsupported shortcuts.
...
for path in "${rr2c_obsolete_paths[@]}"; do
  if [ -e "$path" ] || [ -L "$path" ]; then
    rr2c_bad="$rr2c_bad obsolete:$path"
  fi
done
mapfile -t rr2c_flutter_shells < <(
  find flutter -maxdepth 1 \( -type f -o -type l \) -name '*.sh' -print | LC_ALL=C sort
)
if [ "${#rr2c_flutter_shells[@]}" -ne 1 ] \
  || [ "${rr2c_flutter_shells[0]:-}" != flutter/ndk_arm64.sh ]; then
  rr2c_bad="$rr2c_bad flutter-shell-inventory"
fi
...
# retained upstream workflow references are review provenance rather than latent
# GitHub-hosted execution authority. Dependabot is absent; workflow suffix and
# schema demotion make every retained reference fail closed after a rename.
echo "== (6c-a4) absent/inert GitHub automation authority (R-R1a/R-R2/R-R2d) =="
if ! python3 scripts/verify-github-automation-authority.py --repo . --self-test; then
  echo "  FAIL R-R1a/R-R2/R-R2d: GitHub automation authority is not the exact absent/schema-inert set"
  rc=1
else
  echo "  ok  R-R1a/R-R2/R-R2d Dependabot absent; zero enabled workflows; seven historical references have no active trigger/job schema"
fi
...
# google-services-free; this locks in the residual creds-plist deletion. (The former F-Droid-only
# gms/firebase strip, the spec, and the entitlements R-SV8 comment legitimately NAME the tokens —
# the checks below target the actual creds/endpoint/entitlement, not those mentions.)
rsv8_bad=
