...
# Pre-SEED flutter_tools' resolution (.dart_tool\package_config.json) from the pinned, staged
# res/win/flutter_tools-package_config.json (generated by a REAL Windows flutter resolve against THIS exact
# staged cache; its 97 paths are all C:\Users\builder\...\Pub\Cache + C:\flutter\..., identical in this guest).
# WHY this is REQUIRED, not an optimization: `dart pub get` on a FRESH flutter_tools makes a pub.dev
# advisory/metadata network call that --offline does NOT skip and that is FATAL when it fails -- VERIFIED by
# reproduction in the rdwinvm SSH VM: fresh resolve + working net -> rc=0 "Got dependencies"; the SAME fresh
# resolve with :443 blocked -> rc=69 ("socket is not connected"). A fresh-Win11 guest's TLS handshake to
# pub.dev FAILS ("Handshake error in client (OS Error: ...)"), so that one call killed EVERY provision at this
# line. (An earlier note here claimed the advisory was NON-FATAL/rc=0 -- that was a misread no-op test on an
# ALREADY-resolved flutter_tools; a present .dart_tool makes pub SKIP the net, only a FRESH resolve hits the
# fatal call.) With the resolution pre-seeded the guest NEVER reaches pub.dev for the build. VERIFIED: seed +
# :443 blocked -> "Got dependencies!", rc=0.
$dtDir = 'C:\flutter\packages\flutter_tools\.dart_tool'
New-Item -ItemType Directory -Force -Path $dtDir | Out-Null
Copy-Item (Join-Path $tc 'flutter_tools-package_config.json') (Join-Path $dtDir 'package_config.json') -Force
if (-not (Test-Path (Join-Path $dtDir 'package_config.json'))) { Die 'flutter_tools .dart_tool seed missing -- flutter_tools-package_config.json absent on the TOOLCHAINS CD' }
Log 'seeded flutter_tools .dart_tool from the pinned package_config.json (severs the build from pub.dev)'

Log 'validating the seeded flutter_tools resolution OFFLINE (a 0-network no-op unless the seed is stale)'
...
# the original provision killer; a command-level 2>&1 does NOT prevent it, Start-Process does). With the seed in
# place this is a 0-network no-op ("Got dependencies!"); a STALE seed forces a re-resolve -> the fatal pub.dev
# call -> non-zero exit -> a LOUD, early Die (vs a silently-wrong build). -Wait is REQUIRED for a reliable
# .ExitCode (-PassThru+WaitForExit yields $null -> `-ne 0` always true; verified on rdwinvm: -Wait -> rc=7).
...
if ($pg.ExitCode -ne 0) { Die "flutter_tools offline pub get failed (exit $($pg.ExitCode)) -- the seeded .dart_tool (res/win/flutter_tools-package_config.json) is STALE vs the staged flutter-pub-cache, forcing a re-resolve that cannot reach pub.dev on a fresh-Win11 guest. Regenerate the package_config.json (see res/win/README)." }
