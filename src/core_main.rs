...
pub fn core_main() -> Option<Vec<String>> {
  ...
  --device_name"#
        ...
        // refuse (ui_interface.rs), keeping the flutter FFI signature compiling.
            ...
            return None;
        } else if args[0] == crate::native_video_worker::worker_arg() {
            // Appendix C #2b: hidden same-artifact worker role for hostile-peer
            // native video parsing. It never listens, never joins the session
            // state machine, and exists only as a stdio child of the viewer.
            #[cfg(not(any(target_os = "android", target_os = "ios")))]
            if let Err(e) = crate::native_video_worker::run_worker() {
                log::error!("native video worker failed: {}", e);
            }
            return None;
        } else if args[0] == crate::native_audio_worker::worker_arg() {
            // Appendix C #2b: hidden same-artifact worker role for hostile-peer
            // Opus packet parsing. It never listens, never joins the session
            // state machine, and exists only as a stdio child of the viewer.
            #[cfg(not(any(target_os = "android", target_os = "ios")))]
            if let Err(e) = crate::native_audio_worker::run_worker() {
                log::error!("native Opus worker failed: {}", e);
            }
            return None;
        } else if args[0] == crate::native_clipboard_worker::worker_arg() {
            // Appendix C #2b: hidden same-artifact worker role for hostile-peer
            // normal clipboard parsing/native SET. It never listens and exists
            // only as a stdio child of the viewer or controlled side.
            #[cfg(not(any(target_os = "android", target_os = "ios")))]
            if let Err(e) = crate::native_clipboard_worker::run_worker() {
                log::error!("native clipboard worker failed: {}", e);
            }
            return None;
        } else if {
            #[cfg(target_os = "windows")]
            ...
            // Appendix C #2b: hidden same-artifact worker role for the Windows
            // FreeRDP/COM CLIPRDR bridge. The parent owns the session/channel
            // map; the child owns the native CLIPRDR context and forwards
            // callback-generated ClipboardFile events over bounded stdio frames.
            #[cfg(target_os = "windows")]
            if let Err(e) = clipboard::platform::windows::run_cliprdr_worker() {
                log::error!("native CLIPRDR worker failed: {}", e);
            }
            return None;
        } else if args[0] == hbb_common::compress::worker_arg() {
            // Appendix C #2b: hidden same-artifact worker role for hostile-peer
            // zstd decompression. Local config decompression stays in-process;
            // peer clipboard/cursor/file/terminal payloads use this child.
            #[cfg(not(any(target_os = "android", target_os = "ios")))]
            if let Err(e) = hbb_common::compress::run_worker() {
                log::error!("native zstd worker failed: {}", e);
            }
            return None;
        } else if {
            #[cfg(all(feature = "unix-file-copy-paste", target_family = "unix"))]
            {
                args[0] == crate::native_file_descriptor_worker::worker_arg()
            }
            #[cfg(not(all(feature = "unix-file-copy-paste", target_family = "unix")))]
            {
                false
            }
        } {
            // Appendix C #2b: hidden same-artifact worker role for hostile-peer
            // Unix/macOS file-copy FILEDESCRIPTOR parsing. It never listens and
            // exists only as a stdio child of the file-copy clipboard path.
            #[cfg(all(feature = "unix-file-copy-paste", target_family = "unix"))]
            if let Err(e) = crate::native_file_descriptor_worker::run_worker() {
                log::error!("native file descriptor worker failed: {}", e);
            }
            return None;
        } else if {
            #[cfg(all(feature = "unix-file-copy-paste", target_family = "unix"))]
            {
                args[0] == crate::native_file_contents_worker::worker_arg()
            }
            #[cfg(not(all(feature = "unix-file-copy-paste", target_family = "unix")))]
            {
                false
            ...
            // Appendix C #2b: hidden same-artifact worker role for Unix/macOS
            // file-copy local file-list state and FileContents reads. It never
            // listens and exists only as a stdio child of the file-copy path.
            #[cfg(all(feature = "unix-file-copy-paste", target_family = "unix"))]
            if let Err(e) = crate::native_file_contents_worker::run_worker() {
                log::error!("native file contents worker failed: {}", e);
            }
            return None;
