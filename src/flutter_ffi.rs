...
// block (the cbindgen stubs `get_dart_object`/`new_dart_opaque`) references `Dart_Handle`
// — the allo-isolate pointer type — expecting `use flutter_rust_bridge::*` to bring it in,
// but the locked 1.80.1 crate does not re-export it at its root. The generated bridge does
// `use crate::flutter_ffi::*`, so we supply the alias here. Those stubs are `panic!("dummy
// code")` and are never called, so the exact pointee is immaterial; this only lets the
// flutter-feature FFI `cargo check`/build resolve the type. (bridge_generated.rs is
// git-ignored + regenerated by scripts/flutter-verify.sh, so the fix must live here.)
