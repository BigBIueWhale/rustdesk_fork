...
extern "C" {
    ...
    fn MacVerifyServiceOwnedUnattendedPasswordAuthorizationExternalForm(
        buffer: *const u8,
        len: usize,
    ) -> BOOL;
...
// update_to, extract_update_dmg, extract_dmg, update_extracted (the osascript-admin root DMG
// re-install) — is excised, not disabled. The fork ships signed releases (§12); there is no
// fetch-and-run path. R-A6's Apple-cfg grep pass asserts these symbols are absent.
