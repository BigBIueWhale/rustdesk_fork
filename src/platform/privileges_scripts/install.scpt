on run {daemon_file, agent_file, bundled_service_exec}
  ...
  set expected_bundled_service_exec to "/Applications/RustDesk.app/Contents/MacOS/service"
  ...
  set app_requirement to "=anchor apple generic and certificate leaf[subject.OU] = \"HZF9JMC8YN\" and identifier \"com.carriez.rustdesk\""
  ...
  set verify_installed_app to "if [ " & quoted form of bundled_service_exec & " != " & quoted form of expected_bundled_service_exec & " ]; then exit 1; fi; for app_component in " & quoted form of app_bundle & " " & quoted form of expected_bundled_service_exec & "; do if [ -L \"$app_component\" ] || [ ! -e \"$app_component\" ]; then exit 1; fi; if [ \"$(/usr/bin/stat -f '%Su' \"$app_component\")\" != \"root\" ]; then exit 1; fi; if [ -n \"$(/usr/bin/find \"$app_component\" -prune \\( ! -user root -o -perm +022 \\) -print)\" ]; then exit 1; fi; done; /usr/bin/codesign --verify --deep --strict --all-architectures -R " & quoted form of app_requirement & " " & quoted form of app_bundle & " >/dev/null 2>&1;"

  set verify_bundled_service_exec to "if [ ! -f " & quoted form of bundled_service_exec & " ] || [ ! -x " & quoted form of bundled_service_exec & " ]; then exit 1; fi; if [ -n \"$(/usr/bin/find " & quoted form of bundled_service_exec & " -prune \\( -perm +022 \\) -print)\" ]; then exit 1; fi; /usr/bin/codesign --verify --strict --all-architectures -R " & quoted form of helper_requirement & " " & quoted form of bundled_service_exec & " >/dev/null 2>&1;"

  set install_service_exec to "/bin/rm -f " & quoted form of temp_service_exec & " && /usr/bin/install -o root -g wheel -m 0755 " & quoted form of bundled_service_exec & " " & quoted form of temp_service_exec & " && /bin/chmod -N " & quoted form of temp_service_exec & " && /usr/bin/codesign --verify --strict --all-architectures -R " & quoted form of helper_requirement & " " & quoted form of temp_service_exec & " >/dev/null 2>&1 && /bin/mv -f " & quoted form of temp_service_exec & " " & quoted form of service_exec & " && /bin/chmod -N " & quoted form of service_exec & " && /usr/sbin/chown root:wheel " & quoted form of service_exec & " && /bin/chmod 0755 " & quoted form of service_exec & " && /usr/bin/cmp -s " & quoted form of bundled_service_exec & " " & quoted form of service_exec & ";"

  set verify_service_exec to "for service_component in " & quoted form of helper_dir & " " & quoted form of service_exec & "; do if [ -L \"$service_component\" ]; then exit 1; fi; if [ ! -e \"$service_component\" ]; then exit 1; fi; /bin/chmod -N \"$service_component\" || exit 1; if [ \"$(/usr/bin/stat -f '%Su:%Sg' \"$service_component\")\" != \"root:wheel\" ]; then exit 1; fi; if [ -n \"$(/usr/bin/find \"$service_component\" -prune \\( ! -user root -o ! -group wheel -o -perm +022 \\) -print)\" ]; then exit 1; fi; done; if [ ! -f " & quoted form of service_exec & " ] || [ ! -x " & quoted form of service_exec & " ]; then exit 1; fi; /usr/bin/codesign --verify --strict --all-architectures -R " & quoted form of helper_requirement & " " & quoted form of service_exec & " >/dev/null 2>&1;"

  set verify_current_build_binding to verify_installed_app & "/usr/bin/cmp -s " & quoted form of bundled_service_exec & " " & quoted form of service_exec & ";"
